...
export function MessageBar() {
  ...
  const handleKeyDown = (event) => {
    if (event.key === 'Enter' && !event.shiftKey) {
      ...
      sendMessage();  // Llamar a la función de envío de mensaje
    }
  };
    ...
    <div className={styles.bar}>
      ...
      <div className={styles.bar_inner}>
        <div className={styles.bar_type}>
          <Textarea transparent={true} rows="3" value={typeingMessage?.content || ''} onFocus={() => setIs({ inputing: true })} onBlur={() => setIs({ inputing: false })} placeholder="Enter somthing...." onChange={setMessage} onKeyDown={handleKeyDown}/>
        </div>
